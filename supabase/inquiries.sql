...
create table if not exists public.inquiries (
  ...
  -- 폼 종류(b2b/partnership/media 등) + 세부 문의 유형
  contact_type text,
  inquiry_type text,
  ...
  -- 어느 페이지에서 제출됐는지 (예: /contact, /)
  source_page text,

  status text not null default 'new'
    check (status in ('new', 'read', 'replied', 'spam')),

  -- 알림 메일 발송 결과 (발송 실패해도 접수 자체는 성공 처리)
